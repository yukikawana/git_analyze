...
def normalize(input_tensor, output_tensor, const):
    ...
    return output_tensor / (const *  np.prod(image_dims))


class TotalVariation(Loss):

    def __init__(self, img_input, beta=2., V=80./6.5):
        ...
        self.name = "TV(beta={}, V={}) Loss".format(beta, V)
        self.img = img_input
        self.beta = beta
        self.V = V
    ...
    def build_loss(self):
        ...
        return normalize(self.img, tv, K.pow(self.V, self.beta))


class LPNorm(Loss):

    def __init__(self, img_input, alpha=6., B=80., Bp=80.*2.):
        """
